...
final class TranscriptDeduplicatorTests: XCTestCase {
    ...
    func testTextOnlyRemovesEmbeddedStrongDMExactPhraseAndKeepsNarration() {
        let transcript = Transcript(segments: [
            TranscriptSegment(
                speaker: AudioSource.microphone.rawValue,
                text: "Strong DM's AI team is an example of- Okay, now I'm going to swap to my AirPod."
            ),
            TranscriptSegment(
                speaker: AudioSource.systemAudio.rawValue,
                text: "Companies already use this approach. StrongDM's AI team is an example of this."
            )
        ])

        let result = TranscriptDeduplicator.deduplicateWithReport(transcript)
        let microphone = micText(in: result.transcript) ?? ""

        XCTAssertFalse(microphone.lowercased().contains("ai team is an example"))
        XCTAssertTrue(microphone.contains("Okay, now I'm going to swap to my AirPod."))
        XCTAssertEqual(result.report.removedEmbeddedSpans, 1)
        XCTAssertEqual(result.report.affectedMicrophoneSegmentIndices, [0])
    }

    func testTextOnlyRemovesEmbeddedTravelEchoFromPunctuationPoorSentence() {
        let transcript = Transcript(segments: [
            TranscriptSegment(
                speaker: AudioSource.microphone.rawValue,
                text: "I am going to play the computer speaker Hey chat, hey, I want to do this crazy thing where I have breakfast in Tokyo And then I have lunch in Dubai and then maybe I spend the evening Okay, that is it."
            ),
            TranscriptSegment(
                speaker: AudioSource.systemAudio.rawValue,
                text: "Hey chat. Hey, I want to do this crazy thing where I have breakfast in Tokyo and then I have lunch in Dubai and then maybe I spend the evening."
            )
        ])

        let result = TranscriptDeduplicator.deduplicate(transcript)
        let microphone = micText(in: result) ?? ""

        XCTAssertTrue(microphone.contains("I am going to play the computer speaker"))
        XCTAssertTrue(microphone.contains("Okay, that is it."))
        XCTAssertFalse(microphone.contains("breakfast in Tokyo"))
    }

    func testTimestampedEmbeddedEchoPreservesUniqueWordsEvenAtHighCoverage() {
        let transcript = Transcript(segments: [
            TranscriptSegment(
                speaker: AudioSource.microphone.rawValue,
                startTime: 10,
                endTime: 15,
                text: "My introduction. The launch plan includes legal review and a Friday release. I disagree."
            ),
            TranscriptSegment(
                speaker: AudioSource.systemAudio.rawValue,
                startTime: 10,
                endTime: 15,
                text: "The launch plan includes legal review and a Friday release."
            )
        ])
        ...
        XCTAssertTrue(microphone.contains("My introduction."))
        XCTAssertTrue(microphone.contains("I disagree."))
        XCTAssertFalse(microphone.contains("legal review"))
    }

    func testCustomThresholdAppliesToJaccardSimilarity() {
        let transcript = Transcript(segments: [
            TranscriptSegment(
                speaker: AudioSource.microphone.rawValue,
        ...
        let result = TranscriptDeduplicator.deduplicate(transcript, threshold: 0.65)

        XCTAssertNil(micText(in: result))
    }

    func testTimestampedPathFallsBackSafelyForUntimestampedMicSegment() {
        let transcript = Transcript(segments: [
            TranscriptSegment(
                speaker: AudioSource.systemAudio.rawValue,
                startTime: 20,
                endTime: 24,
                text: "The deployment starts after the final approval."
            ),
            TranscriptSegment(
                speaker: AudioSource.microphone.rawValue,
                text: "The deployment starts after the final approval."
        ...
        XCTAssertNil(micText(in: result.transcript))
        XCTAssertEqual(result.report.removedBySimilarity, 1)
        XCTAssertEqual(result.report.affectedMicrophoneSegmentIndices, [0])
    }
}
